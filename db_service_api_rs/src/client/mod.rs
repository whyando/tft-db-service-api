    ...
    C: Has<XSpanIdString>  + Clone + Send + Sync + 'static,
{
        ...
        context: &C) -> Result<RiotApiResponse, ApiError>
    {
        ...
        let mut uri = format!(
            "{}/{riot_url}",
            self.base_path
            ,riot_url=utf8_percent_encode(&param_riot_url.to_string(), ID_ENCODE_SET)
        );
        ...
        match response.status().as_u16() {
            200 => {
                ...
                let body = serde_json::from_str::<std::collections::HashMap<String, serde_json::Value>>(body)?;
                Ok(RiotApiResponse::Status200
                    (body)
                )
            }
            400 => {
        ...
        context: &C) -> Result<ServerChallengerGetResponse, ApiError>
    {
        ...
        let mut uri = format!(
            "{}/{server}/challenger",
            self.base_path
            ,server=utf8_percent_encode(&param_server.to_string(), ID_ENCODE_SET)
        );
        ...
        context: &C) -> Result<ServerMatchListGetResponse, ApiError>
    {
        ...
        let query_string = {
            let mut query_string = form_urlencoded::Serializer::new("".to_owned());
                query_string.append_pair("Player",
                    &param_player.to_string());
            query_string.finish()
        };
        if !query_string.is_empty() {
            uri += "?";
